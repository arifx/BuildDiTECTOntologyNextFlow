...
g.add((URIRef(fsmon+""), RDF.type, URIRef(obo+"NCIT_C25341")))
g.add((URIRef(obo+"CHMO_0000937"), URIRef(saref+"isMeasuredByDevice"), URIRef(fsmon+"Videometer"))) 
...
g.add((URIRef(obo+"NCIT_C25341"), RDFS.label, Literal("Location")))
...
for index, row in df[["Sample_ID"]].iterrows(): #iter rows of first column, get sampleId
    ...
    for col_index, column in enumerate(df.columns[1:]): # get column name
      ...
      if "Mean" in column:
        ...
        g.add((URIRef(fsmon+column), URIRef(saref+"relatesToProperty"), URIRef(om2+"ViableCount")))
